...
The temperature graphs all look similar, and overlap a lot. The shape is a little surprising, as I didn't expect the graphs to be bimodal. It could potentially be caused by significant differences between summer and winter, with an abrupt shift between the two.
...
```{r weekday.journeys.2v, warning=FALSE, cache=TRUE}
...
```{r bikes.time.name.mv, warning=FALSE, cache=TRUE}
...
For different measures of rain:

```{r rain.rain2.journeys.2v, warning=FALSE, cache=TRUE}
# Note that this doesn't render very well. Horizontal stripes are a rendering
# artifact, not part of the data.
...
```{r fog.journeys.2v, warning=FALSE, cache=TRUE}
...
```{r temp.wind.journeys.2v, cache=TRUE}
...
```{r ggpairs.times, warning=FALSE, cache=TRUE}
...
```{r bikes.time.hists, warning=FALSE, cache=TRUE, fig.width=11, fig.height=11}
...
```{r cor.0900.0930, warning=FALSE, cache=TRUE}
...
```{r cor.0900.0930.rain, warning=FALSE, cache=TRUE}
